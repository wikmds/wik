...
```bash
wikmd@wikmd-Strix-GL504GW-GL504GW:~$ grep '.*' /etc/passwd | cut -d: -f1 | sort
_apt
avahi
backup
bin
...
```
```bash
wikmd@wikmd-Strix-GL504GW-GL504GW:~/pr1 ./banner.sh "wikmd"
+--------+
| wikmd |
+--------+
```
...
```
```bash
wikmd@wikmd-Strix-GL504GW-GL504GW:~/pr1 grep -oE '\b[a-zA-Z_][a-zA-Z0-9_]*\b' hello.c | grep -vE '\b(int|void|return|if|else|for|while|include|stdio)\b' | sort | uniq
h
hello
main
n
...
```bash
wikmd@wikmd-Strix-GL504GW-GL504GW:~/pr1 ./reg.sh banner.sh
wikmd@wikmd-Strix-GL504GW-GL504GW:~/pr1 ls /usr/local/bin
banner.sh  ngrok
```
...
```bash
wikmd@wikmd-System-Product-Name:~/Desktop/confupr$ ./6.sh 
...
```bash
wikmd@wikmd-Strix-GL504GW-GL504GW:~/pr1 ./tar.sh "test" "py"
...
```
```bash
wikmd@wikmd-System-Product-Name:~/Desktop/confupr$ ./6.sh test.c test_new.c
...
```
```bash
wikmd@wikmd-Strix-GL504GW-GL504GW:~/pr1 ./10.sh test
test/3.js
test/1.py
test/1ff
test/2.py
